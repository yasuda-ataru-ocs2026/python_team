...
# ==========================================
# [段落1] 道具の準備とローカル鍵の自動読み込み
# ==========================================
# 💡 `python-dotenv` というライブラリから `load_dotenv` を呼び出します。
# これが実行されると、同じフォルダ内にある隠しファイル `.env` を自動で探し、
# 中に書かれているAPIキーをプログラムが使える状態にスタンバイしてくれます。
...
# ==========================================
# [段落2] 安全にAPIキーをプログラムに取り出す
# ==========================================
# 💡 スンバイされた環境データの中から、"GEMINI_API_KEY" という名前の鍵を抜き取ります。
# コード上に「AIzaSy...」のような生のキーが1文字も残らないため、
# この状態ならGitHubに何回プッシュしても、絶対に怒られることはなくなります！
...
async def upload_receipt(file: UploadFile = File(...)):
    ...
    try:
        # ==========================================
        # [段落3] キーが正しく読み込めているかの事前ガード
        # ==========================================
        # 💡 万が一、ファイルの作成ミスや名前の間違いでキーが空っぽ（None）だった場合、
        # 処理が進んでクラッシュする前に、分かりやすい日本語でエラーを画面に返してくれます。
        if not GEMINI_API_KEY:
            return JSONResponse(status_code=500, content={
                "status": "error",
                "message": "GEMINI_API_KEY が設定されていません。.env ファイルの配置やファイル名を確認してください。"
            })
        ...
        # ==========================================
        # [段落4] 取り出した安全なキーを使ってAI通信を開始
        # ==========================================
        # 💡 [段落2] でローカルから安全に抜き取った `GEMINI_API_KEY` を、Geminiの起動エンジンに渡します。
        # 例の文字化けレシートの呪文を、脳内で完璧にプロファイリングして綺麗なデータに直す命令はそのままです。
        try:
